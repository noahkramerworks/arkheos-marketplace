...
export const PROTOCOL = "freecad-bridge/1";
export const APPLICATION_VERSION = "1.1.3";
export const BRIDGE_VERSION = "0.1.0";
export const API_VERSION = "FreeCAD Python API 1.1.3";
export const BUILD_REVISION = "20260725";
...
export class Coordinator {
  ...
  async handle(req, res) {
    try {
      ...
      if (req.method === "POST" && url.pathname === "/v1/connect") {
        ...
        if (data.protocol !== PROTOCOL || !Number.isInteger(data.pid) || data.applicationVersion !== APPLICATION_VERSION || data.bridgeVersion !== BRIDGE_VERSION || data.apiVersion !== API_VERSION || !String(data.buildRevision || "").startsWith(BUILD_REVISION)) return send(res, 400, { error: "invalid or unsupported native identity" });
  ...
  connection() {
    return [...this.connections.values()].filter((item) => Date.now() - item.lastSeen < 5000).sort((a, b) => b.lastSeen - a.lastSeen)[0] || null;
  }
  async dispatch(operation, input = {}, timeoutMs = this.timeoutMs) {
    if (!this.connection()) throw new Error("FreeCAD native extension is not connected; install it and launch FreeCAD");
    const requestId = randomUUID(); this.queue.push({ protocol: PROTOCOL, requestId, operation, input });
    return await new Promise((resolve, reject) => {
      const timer = setTimeout(() => { this.pending.delete(requestId); reject(new Error(`FreeCAD ${operation} timed out`)); }, timeoutMs);
