...
const documentPath = { type: "string", pattern: "^[A-Za-z]:\\\\.*\\.FCStd$" };
...
export const TOOLS = [
  { name: "bridge_status", description: "Inspect exact FreeCAD 1.1.3, command runtime, owned extension identity, and native connection.", inputSchema: { ...closed, properties: {} } },
  { name: "setup_bridge", description: "Install/remove the owned extension, enroll one user-profile root, launch an enrolled document, or close only the owned clean process.", inputSchema: { ...closed, properties: { action: { enum: ["install_extension", "remove_extension", "enroll_root", "launch_document", "close_owned_process"] }, rootPath: { type: "string", pattern: "^[A-Za-z]:\\\\.+" }, documentPath }, required: ["action"] } },
  { name: "inspect_document", description: "Read bounded native document, revision, dirty, object, parametric, and shape state.", inputSchema: { ...closed, properties: {} } },
  { name: "apply_transaction", description: "Apply one closed clean revision-guarded parametric transaction with checkpoint, native readback, and immutable receipt.", inputSchema: transaction },
  { name: "export_artifact", description: "Export a clean exact-revision document to bridge-owned STEP or STL through the fixed batch adapter.", inputSchema: { ...closed, properties: { documentPath, expectedRevision, format: { enum: ["step", "stl"] } }, required: ["documentPath", "expectedRevision", "format"] } },
  { name: "rollback_receipt", description: "Restore one receipt's exact FCStd checkpoint and verify native reload plus original bytes.", inputSchema: { ...closed, properties: { receiptId: { type: "string", pattern: "^sha256:[a-f0-9]{64}$" } }, required: ["receiptId"] } }
];
...
export async function handleRpc(message, options = defaultRuntime) {
  ...
  if (message.method === "initialize") return { jsonrpc: "2.0", id: message.id, result: { protocolVersion: message.params?.protocolVersion || "2025-06-18", capabilities: { tools: {} }, serverInfo: { name: "freecad-bridge", title: "FreeCAD Bridge", version: "0.1.0" } } };
