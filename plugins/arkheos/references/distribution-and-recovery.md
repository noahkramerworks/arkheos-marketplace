...
Every install or update uses one 15-minute single-use plan bound to product, version, release digest, current Codex build, current marketplace state, and rollback disposition. An explicit version resolves only its immutable version-addressed signed release; omission resolves the signed stable pointer. A receipt records requested effect, staged bytes, manifest acceptance, marketplace registration, plugin installation response, cache readback, and observed discovery separately.
