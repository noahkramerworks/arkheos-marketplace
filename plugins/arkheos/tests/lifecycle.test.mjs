...
test("signed product plan materializes a local marketplace, installs, verifies, exports, and removes", async (t) => {
  ...
  const api = { accountStatus: async () => ({ authorized: true, entitlement: { mutating: true } }), release: async (...args) => (releaseRequests.push(args), { release: selected, verification: { digest: sha256(selected) } }), artifact: async (artifact) => archives.get(artifact.sha256) };
  ...
  const prepared = await operations.prepare({ kind: "install", product: "demo", version: "1.0.0", channel: "stable" });
  assert.equal(prepared.plan.targetVersion, "1.0.0");
  assert.deepEqual(releaseRequests[0], ["demo", "stable", "1.0.0"]);
