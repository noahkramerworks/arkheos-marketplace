...
function run(mode, resultName, factory = false) {
  ...
  const args = [...(factory ? ["--factory-startup"] : [project]), "--background", "--python", driver, "--", mode, project, resultFile];
...
try {
  ...
  if (mutatedSha256 === preSha256 || !written.objects.includes("APIAdmissionObject") || written.dirty) throw new Error("Typed bpy write did not persist");
  const observed = run("inspect", "observed.json");
  if (!observed.objects.includes("APIAdmissionObject") || !observed.blenderVersion.startsWith("5.2.1")) throw new Error("Independent bpy readback failed");
  ...
  if (restored.objects.includes("APIAdmissionObject") || !restored.objects.includes("BaselineObject")) throw new Error("Restored native state differs from baseline");
  console.log(JSON.stringify({
    schema: "blender-bridge/api-admission/v1",
    status: "admitted",
    application: "Blender 5.2.1 LTS",
    contractArtifact: blender,
    typedReads: ["inspect-scene-state", "inspect-dependency-graph", "inspect-render-state"],
    typedWrites: ["apply-scene-transaction", "save-project", "export-artifact"],
    readProbe: { capability: "inspect-scene-state", receiptDigest: digest(created), observationDigest: digest(observed) },
    writeProbe: { capability: "apply-scene-transaction", receiptDigest: digest(written), observationDigest: digest({ mutatedSha256, objects: observed.objects }) },
