...
export class Coordinator {
  ...
  async handle(req, res) {
    try {
      ...
      if (req.method === "POST" && url.pathname === "/v1/connect") {
        ...
        if (data.protocol !== PROTOCOL || !Number.isInteger(data.pid) || data.applicationVersion !== APPLICATION_VERSION || data.bridgeVersion !== BRIDGE_VERSION || data.apiVersion !== API_VERSION || data.versionInt !== 40200) return send(res, 400, { error: "invalid or unsupported native identity" });
  ...
  async dispatch(operation, input = {}, timeoutMs = this.timeoutMs) {
    if (!this.connection()) throw new Error("QGIS extension is not connected");
